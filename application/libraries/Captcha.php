...
class Captcha {
    ...
    function __construct() {
        ...
        $this->captcha_config = $this->ci->config->item('captcha');
    }
    ...
    function create_captcha($data = '') {
        $defaults = array(
            ...
            'font_path' => '',
            'expiration' => 7200,
            'char_colors' => array('D08504','29407C','660000','003D4D','00583D','003D4D','000000'),
            'line_colors' => array('DD6666','66DD66','6666DD','DDDD66','DD66DD','66DDDD','666666'),
        );
        $data = ($data=='') ? $this->captcha_config : $data;
        ...
        for ($i = 0; $i < ($circles * $points) - 1; $i++) {
            ...
            imageline($im, $x, $y, $x1, $y1, hexdec($line_colors[rand(0,sizeof($line_colors)-1)]));
            $theta = $theta - $thetac;
        }
        ...
        for ($i = 0; $i < strlen($word); $i++) {
            if ($use_font == FALSE) {
                ...
                imagestring($im, $font_size, $x, $y, substr($word, $i, 1), hexdec($char_colors[rand(0,sizeof($line_colors)-1)]));
                $x += ($font_size*2);
            }
            else {
                $y = rand(($img_height/2)+3, $img_height-3);
                //$y = $img_height/2;
                imagettftext($im, $font_size, $angle, $x, $y, hexdec($char_colors[rand(0,sizeof($line_colors)-1)]), $font_path, substr($word, $i, 1));
                $x += $font_size;
            }
        }
...
/* Location: ./app/library/Captcha.php */