...
class Akismet {
    ...
    private $akismet_server = 'rest.akismet.com';
    private $api_port = 80;
    ...
    {
        if ($this->is_error('AKISMET_INVALID_KEY'))
        {
            log_message('error', 'AKISMET :: Theres a problem with the api key');
        }
        elseif ($this->is_error('AKISMET_RESPONSE_FAILED'))
        {
            log_message('error', 'AKISMET :: Looks like the servers not responding');
        }
        elseif ($this->is_error('AKISMET_SERVER_NOT_FOUND'))
        {
    ...
    public function _is_valid_api_key($key)
    {
