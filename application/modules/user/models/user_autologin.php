...
class User_Autologin extends CI_Model
{
    ...
    function get($user_id, $key)
    {
        ...
        $this->db->select(self::TABLE_USERS . '.email');
        $this->db->select(self::TABLE_USERS . '.user_group');
        ...
        $this->db->join(self::TABLE_PROFILE, self::TABLE_USERS . '.id = ' . self::TABLE_PROFILE . '.user_id', 'left');
